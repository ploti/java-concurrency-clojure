...
(defn notify-msg
  ...
  (let [buffer (state :buffer)
        bill (state :bill)
        required (get bill ware)
        at-warehouse (get buffer ware 0)
        remaining (min (- required at-warehouse) @storage-atom)]
    (if (> remaining 0)
      (try
        ;; update
        (swap! storage-atom - remaining)
        ;; prints current state
        (println (.format (new java.text.SimpleDateFormat "hh.mm.ss.SSS") (new java.util.Date)) "|"
                 ware "amount: " remaining)
        (let [buffer-new (update buffer ware + remaining)
              state-int (assoc state :buffer buffer-new)
              state-new ((fn [worker-state]
                           (let [buffer (worker-state :buffer)
                                 bill (worker-state :bill)
                                 amount (worker-state :amount)
                                 target-storage (worker-state :target-storage)
                                 duration (worker-state :duration)]
                             (if (= buffer bill)
                               (let []
                                 (Thread/sleep duration)
                                 (send (target-storage :worker) supply-msg amount)
                               ...
                               {:buffer buffer
                                ...
                                :target-storage target-storage
                                :amount amount
                                :duration duration}))) state-int)]
          state-new)
        ;; state in case of no update
        (catch IllegalStateException _ state)) state)))
